...
const PLATFORMS = {
	facebook: {
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
				<path d="M10 1.25C5.17 1.25 1.25 5.17 1.25 10c0 4.36 3.21 7.98 7.4 8.66v-6.14H6.46V10h2.19V8.27c0-2.16 1.29-3.35 3.25-3.35.94 0 1.93.17 1.93.17v2.12h-1.09c-1.07 0-1.4.66-1.40 1.34V10h2.39l-.38 2.52h-2.01v6.14c4.19-.68 7.41-4.3 7.41-8.66 0-4.83-3.92-8.75-8.75-8.75z"/>
			</svg>
		)
	},
	twitter: {
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
				<path d="M13.5 3.5h1.5l-3.3 3.7L15.5 16h-3l-2.4-3.1L7.5 16H6l3.5-4L6 3.5h3.1l2.2 2.9 2.7-2.9zm-.5 11.3h.8L7.7 4.3H6.9l6.1 10.5z"/>
			</svg>
		)
	},
	linkedin: {
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
				<path d="M5.5 7.8H3v8h2.5v-8zM4.2 3.5c-.8 0-1.4.6-1.4 1.4s.6 1.4 1.4 1.4 1.4-.6 1.4-1.4-.6-1.4-1.4-1.4zM17 15.8h-2.5v-3.9c0-.9 0-2.1-1.3-2.1-1.3 0-1.5 1-1.5 2v4h-2.5v-8h2.4v1.1h.0c.3-.6 1.1-1.3 2.3-1.3 2.5 0 2.9 1.6 2.9 3.7v4.5h.2z"/>
			</svg>
		)
	},
	pinterest: {
	...
	email: {
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
				<path d="M15 4H5c-.83 0-1.49.67-1.49 1.5L3.5 14c0 .83.67 1.5 1.5 1.5h10c.83 0 1.5-.67 1.5-1.5V5.5c0-.83-.67-1.5-1.5-1.5zm0 3l-5 3.12L5 7V5.5l5 3.12 5-3.12V7z"/>
			</svg>
		)
	},
	whatsapp: {
	...
	telegram: {
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
				<path d="M9.95 1.67A8.33 8.33 0 001.67 10c0 4.6 3.73 8.33 8.33 8.33s8.33-3.73 8.33-8.33-3.73-8.33-8.33-8.33zm4.14 5.52c.08-.01.27.02.39.12.08.06.13.16.14.27.01.08.03.26.02.39-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.21-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.13-.05-.18s-.14-.03-.21-.02c-.09.02-1.49.95-4.22 2.79-.4.28-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.15 3.35-1.36 3.73-1.36z"/>
			</svg>
		)
	},
};
...
/**
 * Edit component for Social Sharing block.
 *
...
export default function Edit({ attributes, setAttributes }) {
	const { platforms, showLabels, openInNewTab } = attributes;
	...
	return (
		<>
			<div {...blockProps}>
				<div className="social-sharing-preview">
					{platforms.length === 0 ? (
						<p className="social-sharing-empty">
							{__('Select platforms to share in the inspector panel', 'prolific-blocks')}
						</p>
					) : (
						<div className="social-sharing-buttons">
							{platforms.map((platform) => {
								...
								return (
									<button
										key={platform}
										className={`social-share-button social-${platform}`}
										type="button"
									>
										<span className="share-icon">{platformData.icon}</span>
										{showLabels && (
											<span className="share-label">{platformData.label}</span>
										)}
									</button>
								);
							})}
			...
			<InspectorControls>
				...
				<PanelBody title={__('Display Settings', 'prolific-blocks')} initialOpen={true}>
					<ToggleControl
						label={__('Show Labels', 'prolific-blocks')}
						help={__('Display text labels alongside icons', 'prolific-blocks')}
						checked={showLabels}
						onChange={(value) => setAttributes({ showLabels: value })}
					/>
					<ToggleControl
						label={__('Open in New Tab', 'prolific-blocks')}
						help={__('Open sharing links in a new browser tab', 'prolific-blocks')}
